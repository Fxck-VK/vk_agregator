...
Production VPS configuration uses a separate tracked template:
...
Then replace every `CHANGE_ME_*` value in the real server `.env`. Keep
`APP_ENV=production` and `APP_ENV_FILE=.env` on the VPS. `docker-compose.prod.yml`
uses `APP_ENV_FILE` to choose the runtime env file; CI overrides it to
`.env.prod.example` only for config validation.

Required production secrets/runtime endpoints:
...
| `VK_SECRET` | VK Callback API secret key | Required in `APP_ENV=production` |
| `VK_CONFIRMATION_TOKEN` | VK Callback API confirmation string | Required in `APP_ENV=production`; must not stay `dev-confirmation` |
| `VK_APP_SECRET` | VK Mini App protected key | Required in `APP_ENV=production`; protects launch params |
...
| `OPENAI_API_KEY` | OpenAI | Required only when OpenAI provider/moderation/scanner is enabled |
| `ADMIN_TOKEN` | Generated by us | Required in `APP_ENV=production`; use a long random value |
...
Do not run app binaries with `.env.prod.example`; copy it to real `.env` and
...
Build and validate the production compose file without real secrets:

```bash
APP_ENV_FILE=.env.example docker compose --env-file .env.example -f docker-compose.prod.yml config
APP_ENV_FILE=.env.example docker compose --env-file .env.example -f docker-compose.prod.yml build
```
...
docker compose -f docker-compose.prod.yml up -d api worker provider-webhook miniapp reverse-proxy
```
...
| Public hostname | Service when `cloudflared` runs in `docker-compose.prod.yml` | Service when `cloudflared` runs on the VPS host |
|-----------------|-------------------------------------------------------------|-----------------------------------------------|
| `vk.neiirohub.ru` | `http://reverse-proxy:80` | `http://127.0.0.1:8088` |
| `app.neiirohub.ru` | `http://reverse-proxy:80` | `http://127.0.0.1:8088` |
| `neiirohub.ru` | `http://reverse-proxy:80` | `http://127.0.0.1:8088` |
...
| `@` | CNAME/flattened CNAME | `<tunnel-id>.cfargotunnel.com`, only if the root YooKassa compatibility route is used |

Run cloudflared from compose:

```bash
# Real .env must contain CLOUDFLARED_TUNNEL_TOKEN.
docker compose -f docker-compose.prod.yml --profile cloudflare up -d cloudflared
```
...
1. Stop if tracked worktree files are dirty, unless `-AllowDirty` /
   `--allow-dirty` is explicitly passed.
2. `git fetch`, checkout the target branch and `git pull --ff-only`.
3. Validate `docker-compose.prod.yml` with the selected env file.
4. Optionally run backup profile first (`-BackupBeforeDeploy` /
   `--backup-before-deploy`).
5. Start stateful dependencies: Postgres, Redis, MinIO.
6. Build runtime images.
7. Recreate and run the one-shot `migrate` service.
8. Start `api`, `worker`, `provider-webhook`, `miniapp`, `reverse-proxy` and
   optional `cloudflared`.
9. Check local health endpoints and print `docker compose ps`.

Useful options:

| PowerShell | Bash | Purpose |
|------------|------|---------|
| `-ImageTag <tag>` | `--image-tag <tag>` | Build and run release images with an explicit tag for later rollback |
| `-SkipPull` | `--skip-pull` | Deploy the already checked-out code |
| `-SkipBuild` | `--skip-build` | Restart with existing images |
...
| Runtime images | Build every release with an explicit `IMAGE_TAG` such as a git SHA or release tag. Keep at least the previous known-good image tag on the VPS or registry |
...
2. run Postgres and MinIO backups first by default;
3. set `IMAGE_TAG` to the previous known-good runtime image tag;
4. start `api`, `worker`, `provider-webhook`, `miniapp`, `reverse-proxy` with
   `--no-build --no-deps`;
