...
- [~] Mini App/VK bot top-up backend flow: payment domain/migration/config foundation, tx-aware `billingservice.GrantWith`, provider port, mock/YooKassa adapters, payment service, product catalog seed, 54-FZ receipt snapshots, Mini App/VK Bot purchase intent UX, authenticated payment-intent/history endpoints, async YooKassa webhook-to-ledger top-up processing, stale-intent reconciliation and conservative manual full-refund MVP exist; still add product catalog admin management, partial/automatic refund lot/FIFO policy, Mini App payment-history UI wiring and live YooKassa smoke.
