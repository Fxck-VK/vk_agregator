...
describe("ModelCapabilitiesDetails", () => {
  it("renders application support separately from native API support", () => {
    render(<ModelCapabilitiesDetails capabilities={mixedTextCapabilities} />);

    const app = screen.getByRole("region", { name: "В этом интерфейсе" });
    const api = screen.getByRole("region", { name: "В API провайдера" });

    expect(within(app).getAllByText("Нет")).toHaveLength(3);
    expect(within(app).queryByText("Не подтверждено")).toBeNull();
    expect(within(api).getByText("Не подтверждено")).toBeInTheDocument();
    expect(within(api).getAllByText(/Да, максимум не подтверждён/)).toHaveLength(2);
    expect(within(api).queryByText("Нет")).toBeNull();
  });
