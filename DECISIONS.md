...
## ADR-015 - Merge VK bot integration branch with Mini App fastlife_dev

Status: accepted

Date: 2026-06-06

Context: `feature/integration-web-backend` contains the colleague VK bot branch
work through `44df8d4`: VK menu hardening, callback UX, Redis dialog mode,
anti-spam, durable dialog context, shared referrals and migrations
`000005`-`000007`. `fastlife_dev` contains Mini App PR-6 through PR-16.x plus
targeted backend work: estimate endpoint, Mini App chat BFF, async provider
submit timeout, reservation release on terminal provider failure and launch
param clock-skew tolerance.

Decision: merge `fastlife_dev` into the updated
`feature/integration-web-backend` with union semantics. The merge must preserve
both inbound paths and keep provider access behind the job/worker path:

- VK inbound keeps webhook/menu/callback/dialog-state/anti-spam/referral
  behavior and still creates jobs instead of calling providers directly.
- Mini App BFF keeps launch-param auth, rate limiting, estimate, jobs, chat,
  balance and artifact routes without frontend-side billing or provider calls.
- Worker keeps colleague text-context preparation/completion and our provider
  Submit/Poll timeout/release behavior.
- Billing remains append-only: grants, top-ups/referrals, reserve, capture,
  release and refund continue through billing repositories and ledger entries.
- Public Mini App chat model labeling remains `ChatGPT`; raw DeepInfra model
  names stay implementation configuration.

Consequences: the integrated branch is the new place to validate both product
