# 57. Seedance 2.0 Mini — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** подключить Seedance 2.0 Mini через выбранный маршрут APIMart.
...
- [Карточка Study24](https://study24.ai/chat/seedance_mini)
- [Документация seedance-2-0/generation](https://docs.apimart.ai/ru/api-reference/videos/seedance-2-0/generation)
...
| Поле | Значение |
| --- | --- |
| Название Study24 | Seedance 2.0 Mini |
| Предлагаемый public ID | `seedance_2_0_mini` |
| APIMart model ID | `seedance-2.0-mini` |
| Метод | `POST https://api.apimart.ai/v1/videos/generations` |
| Флаг | `FEATURE_APIMART_SEEDANCE_2_0_MINI_ENABLED` |
| Соответствие | Название модели сопоставлено с каталогом/контрактом APIMart; доступ рабочего ключа ещё не проверен |

Создать отдельный public ID Mini; не направлять его на Standard при ошибке.
...
4–15 с; только 480p/720p. Старт 5/10/15 с T2V/I2V. До 9 изображений/3 видео/3 аудио после расширения media contract.
...
Без video: $0.01056/с 480p, $0.02288/с 720p. С video: $0.0064/$0.01384 × (вход+выход).
...
- Создать fixture `internal/adapter/provider/apimart/testdata/contracts/seedance_2_0_mini.request.json`.
- Создать `internal/adapter/provider/apimart/model_seedance_2_0_mini_test.go` с именем теста `TestModel_seedance_2_0_mini`.

## Порядок реализации

- [ ] **1. Контракт.** В B0 проверить `seedance-2.0-mini`, записать разрешённые поля/операции и очищенный fixture указанного запроса. Для спорной версии сохранить предлагаемое полное название в UI и закрыть rollout до подтверждения выбора. Не выводить статус «подключено» по одному наличию в прайсе.
- [ ] **2. Адаптер.** Реализовать serializer и capability для `seedance-2.0-mini` по указанному JSON; локально проверять лимиты. Разобрать описанную форму результата. В тестовом HTTP server проверить exact endpoint/body; неподдерживаемое значение не должно вызывать HTTP.
- [ ] **3. Каталог и стоимость.** Добавить/обновить public ID `seedance_2_0_mini`, backend readiness и `FEATURE_APIMART_SEEDANCE_2_0_MINI_ENABLED` в config wiring. Создать отдельные ключи всех открываемых вариантов; определить exact rates и reserve/capture fixture. Сохранить model ID и тариф в Job snapshot.
...
- [ ] **6. Выпуск.** Пройти preflight, тесты и ограниченный отдельно разрешённый canary именно этой конфигурации; сверить provider cost и ledger. До этого `FEATURE_APIMART_SEEDANCE_2_0_MINI_ENABLED` выключен. Отключение запрещает новые submit, но не мешает завершить старые Jobs.

## Приёмочные сценарии

- [ ] 5 с 480p=$0.0528; 5 с 720p=$0.1144.
- [ ] 1080p и 4K отклоняются.
- [ ] 5+5 с 720p с video=$0.1384, до учёта возможных token-особенностей.
- [ ] `FEATURE_APIMART_SEEDANCE_2_0_MINI_ENABLED=false` → модель недоступна для нового Job; ключ/остаток провайдера не раскрываются клиенту.
...
- [ ] Стадия 2: video/audio references по матрице 2.0 Mini; проверка input tariff и максимума 15 секунд. 1080p/4K не открывать даже если они доступны у Standard.

## Команды проверки при реализации

```powershell
go test ./internal/adapter/provider/apimart -run '^TestModel_seedance_2_0_mini$' -count=1
