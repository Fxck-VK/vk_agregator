# 35. Gemini 2.5 Flash Lite — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** подключить Gemini 2.5 Flash Lite через выбранный маршрут APIMart.
...
- [Карточка Study24](https://study24.ai/chat/gemini25_flash_lite)
...
| Поле | Значение |
| --- | --- |
| Название Study24 | Gemini 2.5 Flash Lite |
| Предлагаемый public ID | `gemini_2_5_flash_lite` |
| APIMart model ID | `gemini-2.5-flash-lite` |
| Метод | `POST https://api.apimart.ai/v1/chat/completions` |
| Флаг | `FEATURE_APIMART_GEMINI_2_5_FLASH_LITE_ENABLED` |
...
$0.08 / $0.32 за 1 млн входных / выходных токенов (обычный контекст, без инструментов и скидки кэша).
...
- Создать fixture `internal/adapter/provider/apimart/testdata/contracts/gemini_2_5_flash_lite.request.json`.
- Создать `internal/adapter/provider/apimart/model_gemini_2_5_flash_lite_test.go` с именем теста `TestModel_gemini_2_5_flash_lite`.

## Порядок реализации

- [ ] **1. Контракт.** В B0 проверить `gemini-2.5-flash-lite`, записать разрешённые поля/операции и очищенный fixture указанного запроса. Для спорной версии сохранить предлагаемое полное название в UI и закрыть rollout до подтверждения выбора. Не выводить статус «подключено» по одному наличию в прайсе.
- [ ] **2. Адаптер.** Реализовать serializer и capability для `gemini-2.5-flash-lite` по указанному JSON; локально проверять лимиты. Разобрать описанную форму результата. В тестовом HTTP server проверить exact endpoint/body; неподдерживаемое значение не должно вызывать HTTP.
- [ ] **3. Каталог и стоимость.** Добавить/обновить public ID `gemini_2_5_flash_lite`, backend readiness и `FEATURE_APIMART_GEMINI_2_5_FLASH_LITE_ENABLED` в config wiring. Создать отдельные ключи всех открываемых вариантов; определить exact rates и reserve/capture fixture. Сохранить model ID и тариф в Job snapshot.
...
- [ ] **6. Выпуск.** Пройти preflight, тесты и ограниченный отдельно разрешённый canary именно этой конфигурации; сверить provider cost и ledger. До этого `FEATURE_APIMART_GEMINI_2_5_FLASH_LITE_ENABLED` выключен. Отключение запрещает новые submit, но не мешает завершить старые Jobs.

## Приёмочные сценарии

- [ ] Запрос с public_id=gemini_2_5_flash_lite резолвится строго в APIMart/gemini-2.5-flash-lite.
- [ ] При входе 1000 и выходе 1000 токенов себестоимость равна $0.0004; применение скидки второй раз запрещено.
- [ ] Ответ без choices[0].message.content или с некорректным usage не должен пройти как успешная бесплатная генерация.
- [ ] `FEATURE_APIMART_GEMINI_2_5_FLASH_LITE_ENABLED=false` → модель недоступна для нового Job; ключ/остаток провайдера не раскрываются клиенту.
...
```powershell
go test ./internal/adapter/provider/apimart -run '^TestModel_gemini_2_5_flash_lite$' -count=1
