# 21. ChatGPT 5.4 — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** подключить ChatGPT 5.4 через выбранный маршрут APIMart.
...
- [Карточка Study24](https://study24.ai/chat/chat_gpt5_4)
...
| Поле | Значение |
| --- | --- |
| Название Study24 | ChatGPT 5.4 |
| Предлагаемый public ID | `gpt_5_4` |
| APIMart model ID | `gpt-5.4` |
| Метод | `POST https://api.apimart.ai/v1/chat/completions` |
| Флаг | `FEATURE_APIMART_GPT_5_4_ENABLED` |
...
$2 / $12 за 1 млн входных / выходных токенов (обычный контекст, без инструментов и скидки кэша).
...
- Создать fixture `internal/adapter/provider/apimart/testdata/contracts/gpt_5_4.request.json`.
- Создать `internal/adapter/provider/apimart/model_gpt_5_4_test.go` с именем теста `TestModel_gpt_5_4`.

## Порядок реализации

- [ ] **1. Контракт.** В B0 проверить `gpt-5.4`, записать разрешённые поля/операции и очищенный fixture указанного запроса. Для спорной версии сохранить предлагаемое полное название в UI и закрыть rollout до подтверждения выбора. Не выводить статус «подключено» по одному наличию в прайсе.
- [ ] **2. Адаптер.** Реализовать serializer и capability для `gpt-5.4` по указанному JSON; локально проверять лимиты. Разобрать описанную форму результата. В тестовом HTTP server проверить exact endpoint/body; неподдерживаемое значение не должно вызывать HTTP.
- [ ] **3. Каталог и стоимость.** Добавить/обновить public ID `gpt_5_4`, backend readiness и `FEATURE_APIMART_GPT_5_4_ENABLED` в config wiring. Создать отдельные ключи всех открываемых вариантов; определить exact rates и reserve/capture fixture. Сохранить model ID и тариф в Job snapshot.
...
- [ ] **6. Выпуск.** Пройти preflight, тесты и ограниченный отдельно разрешённый canary именно этой конфигурации; сверить provider cost и ledger. До этого `FEATURE_APIMART_GPT_5_4_ENABLED` выключен. Отключение запрещает новые submit, но не мешает завершить старые Jobs.

## Приёмочные сценарии

- [ ] Запрос с public_id=gpt_5_4 резолвится строго в APIMart/gpt-5.4.
- [ ] При входе 1000 и выходе 1000 токенов себестоимость равна $0.014; применение скидки второй раз запрещено.
- [ ] Ответ без choices[0].message.content или с некорректным usage не должен пройти как успешная бесплатная генерация.
- [ ] `FEATURE_APIMART_GPT_5_4_ENABLED=false` → модель недоступна для нового Job; ключ/остаток провайдера не раскрываются клиенту.
...
```powershell
go test ./internal/adapter/provider/apimart -run '^TestModel_gpt_5_4$' -count=1
