# 31. DeepSeek V4 — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** при выборе этого плана добавить отдельный маршрут DeepSeek V4 Flash через APIMart, сохранив текущий default chat через DeepInfra.
...
Сверка 08.09.2026, HEAD `cb34ded57`: `internal/service/providermodels/registry.go` содержит `chatgpt` / NeiroHub Chat → DeepInfra / `deepseek-ai/DeepSeek-V4-Flash`. Web chat использует default alias без выбора model_id. Это существующая текстовая функция, а не готовый маршрут APIMart.

- [x] Текущий default chat и общий Job/worker/Artifact путь существуют; локальные registry/modelcatalog/worker tests прошли.
- [ ] Отдельного public ID `deepseek_v4_flash`, APIMart text serializer, его token pricing и выбора в Web пока нет — это оставшийся объём данного плана после T.

Не переименовывать и не перенацеливать `chatgpt`; сохранить прежнюю политику его цены. Сходство семейства модели не подтверждает равенство версии Study24 или контракта двух провайдеров. Live-доступность при актуализации не проверялась.
...
- [Карточка Study24](https://study24.ai/chat/deepseek_v4_lite)
...
| Поле | Значение |
| --- | --- |
| Название Study24 | DeepSeek V4 |
| Предлагаемый public ID | `deepseek_v4_flash` |
| APIMart model ID | `deepseek-v4-flash` |
| Метод | `POST https://api.apimart.ai/v1/chat/completions` |
| Флаг | `FEATURE_APIMART_DEEPSEEK_V4_FLASH_ENABLED` |
| Соответствие | Условное: версия/режим уточнены в этом плане; тождество Study24 не доказано |

Study24 использует общее название DeepSeek V4 (URL содержит lite). Flash — кандидат, равенство версии не доказано. Вне пика: $0.1714284 / $0.5142856; резерв по пиковому тарифу.
...
$0.3428568 / $1.0285712 за 1 млн входных / выходных токенов (обычный контекст, без инструментов и скидки кэша).
...
- Создать fixture `internal/adapter/provider/apimart/testdata/contracts/deepseek_v4_flash.request.json`.
- Создать `internal/adapter/provider/apimart/model_deepseek_v4_flash_test.go` с именем теста `TestModel_deepseek_v4_flash`.

## Порядок реализации

- [ ] **1. Контракт.** В B0 проверить `deepseek-v4-flash`, записать разрешённые поля/операции и очищенный fixture указанного запроса. Для спорной версии сохранить предлагаемое полное название в UI и закрыть rollout до подтверждения выбора. Не выводить статус «подключено» по одному наличию в прайсе.
- [ ] **2. Адаптер.** Реализовать serializer и capability для `deepseek-v4-flash` по указанному JSON; локально проверять лимиты. Разобрать описанную форму результата. В тестовом HTTP server проверить exact endpoint/body; неподдерживаемое значение не должно вызывать HTTP.
- [ ] **3. Каталог и стоимость.** Добавить/обновить public ID `deepseek_v4_flash`, backend readiness и `FEATURE_APIMART_DEEPSEEK_V4_FLASH_ENABLED` в config wiring. Создать отдельные ключи всех открываемых вариантов; определить exact rates и reserve/capture fixture. Сохранить model ID и тариф в Job snapshot.
...
- [ ] **6. Выпуск.** Пройти preflight, тесты и ограниченный отдельно разрешённый canary именно этой конфигурации; сверить provider cost и ledger. До этого `FEATURE_APIMART_DEEPSEEK_V4_FLASH_ENABLED` выключен. Отключение запрещает новые submit, но не мешает завершить старые Jobs.

## Приёмочные сценарии

- [ ] Запрос с public_id=deepseek_v4_flash резолвится строго в APIMart/deepseek-v4-flash.
- [ ] При входе 1000 и выходе 1000 токенов себестоимость равна $0.001371428; применение скидки второй раз запрещено.
- [ ] Ответ без choices[0].message.content или с некорректным usage не должен пройти как успешная бесплатная генерация.
- [ ] `FEATURE_APIMART_DEEPSEEK_V4_FLASH_ENABLED=false` → модель недоступна для нового Job; ключ/остаток провайдера не раскрываются клиенту.
...
```powershell
go test ./internal/adapter/provider/apimart -run '^TestModel_deepseek_v4_flash$' -count=1
