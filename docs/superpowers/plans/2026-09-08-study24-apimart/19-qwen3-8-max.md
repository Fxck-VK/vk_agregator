# 19. Qwen 3.8 Max — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** подключить Qwen 3.8 Max через выбранный маршрут APIMart.
...
- [Карточка Study24](https://study24.ai/chat/qwen_max)
- [Документация general/chat-completions](https://docs.apimart.ai/ru/api-reference/texts/general/chat-completions)
- [Документация qwen3.8-max/guide](https://docs.apimart.ai/ru/api-reference/texts/qwen3.8-max/guide)
- [Документация qwen3.8-max/pricing](https://docs.apimart.ai/ru/api-reference/texts/qwen3.8-max/pricing)
...
| Поле | Значение |
| --- | --- |
| Название Study24 | Qwen 3.8 Max |
| Предлагаемый public ID | `qwen3_8_max` |
| APIMart model ID | `qwen3.8-max` |
| Метод | `POST https://api.apimart.ai/v1/chat/completions` |
| Флаг | `FEATURE_APIMART_QWEN3_8_MAX_ENABLED` |
| Соответствие | Название модели сопоставлено с каталогом/контрактом APIMart; доступ рабочего ключа ещё не проверен |

Thinking нельзя отключить. Встроенные инструменты доступны через Responses и оплачиваются отдельно; в первой версии только текст. В расчёт включить reasoning tokens. Источник ставки — pricing.effective_rates.
...
$1.3714288 / $4.1142856 за 1 млн входных / выходных токенов (обычный контекст, без инструментов и скидки кэша).
...
- Создать fixture `internal/adapter/provider/apimart/testdata/contracts/qwen3_8_max.request.json`.
- Создать `internal/adapter/provider/apimart/model_qwen3_8_max_test.go` с именем теста `TestModel_qwen3_8_max`.

## Порядок реализации

- [ ] **1. Контракт.** В B0 проверить `qwen3.8-max`, записать разрешённые поля/операции и очищенный fixture указанного запроса. Для спорной версии сохранить предлагаемое полное название в UI и закрыть rollout до подтверждения выбора. Не выводить статус «подключено» по одному наличию в прайсе.
- [ ] **2. Адаптер.** Реализовать serializer и capability для `qwen3.8-max` по указанному JSON; локально проверять лимиты. Разобрать описанную форму результата. В тестовом HTTP server проверить exact endpoint/body; неподдерживаемое значение не должно вызывать HTTP.
- [ ] **3. Каталог и стоимость.** Добавить/обновить public ID `qwen3_8_max`, backend readiness и `FEATURE_APIMART_QWEN3_8_MAX_ENABLED` в config wiring. Создать отдельные ключи всех открываемых вариантов; определить exact rates и reserve/capture fixture. Сохранить model ID и тариф в Job snapshot.
...
- [ ] **6. Выпуск.** Пройти preflight, тесты и ограниченный отдельно разрешённый canary именно этой конфигурации; сверить provider cost и ledger. До этого `FEATURE_APIMART_QWEN3_8_MAX_ENABLED` выключен. Отключение запрещает новые submit, но не мешает завершить старые Jobs.

## Приёмочные сценарии

- [ ] Запрос с public_id=qwen3_8_max резолвится строго в APIMart/qwen3.8-max.
- [ ] При входе 1000 и выходе 1000 токенов себестоимость равна $0.0054857144; применение скидки второй раз запрещено.
- [ ] Если ответ содержит только reasoning_content и пустой content, не выдавать рассуждение как ответ; завершить управляемой ошибкой.
- [ ] `FEATURE_APIMART_QWEN3_8_MAX_ENABLED=false` → модель недоступна для нового Job; ключ/остаток провайдера не раскрываются клиенту.
...
```powershell
go test ./internal/adapter/provider/apimart -run '^TestModel_qwen3_8_max$' -count=1
