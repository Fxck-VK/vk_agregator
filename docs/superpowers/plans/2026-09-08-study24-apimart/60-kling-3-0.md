# 60. Kling 3.0 — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** подключить Kling 3.0 через выбранный маршрут APIMart.
...
- [Карточка Study24](https://study24.ai/chat/kling_3)
- [Документация kling-v3/generation](https://docs.apimart.ai/ru/api-reference/videos/kling-v3/generation)
...
| Поле | Значение |
| --- | --- |
| Название Study24 | Kling 3.0 |
| Предлагаемый public ID | `kling_3_0` |
| APIMart model ID | `kling-v3` |
| Метод | `POST https://api.apimart.ai/v1/videos/generations` |
| Флаг | `FEATURE_APIMART_KLING_3_0_ENABLED` |
| Соответствие | Название модели сопоставлено с каталогом/контрактом APIMart; доступ рабочего ключа ещё не проверен |

Сначала один шот; multi_shot и 4K включать отдельными вариантами после проверки цены и media contract.
...
3–15 с; mode std/pro/4k определяет разрешение. 1–2 изображения для первого/последнего кадра. Старт 5/10/15 с, std/pro, audio выбран явно.
...
720p std: $0.0672/с без звука, $0.1008 со звуком. 1080p pro: $0.0896/$0.1344. 4K: $0.42856/с с/без звука.
...
- Создать fixture `internal/adapter/provider/apimart/testdata/contracts/kling_3_0.request.json`.
- Создать `internal/adapter/provider/apimart/model_kling_3_0_test.go` с именем теста `TestModel_kling_3_0`.

## Порядок реализации

- [ ] **1. Контракт.** В B0 проверить `kling-v3`, записать разрешённые поля/операции и очищенный fixture указанного запроса. Для спорной версии сохранить предлагаемое полное название в UI и закрыть rollout до подтверждения выбора. Не выводить статус «подключено» по одному наличию в прайсе.
- [ ] **2. Адаптер.** Реализовать serializer и capability для `kling-v3` по указанному JSON; локально проверять лимиты. Разобрать описанную форму результата. В тестовом HTTP server проверить exact endpoint/body; неподдерживаемое значение не должно вызывать HTTP.
- [ ] **3. Каталог и стоимость.** Добавить/обновить public ID `kling_3_0`, backend readiness и `FEATURE_APIMART_KLING_3_0_ENABLED` в config wiring. Создать отдельные ключи всех открываемых вариантов; определить exact rates и reserve/capture fixture. Сохранить model ID и тариф в Job snapshot.
...
- [ ] **6. Выпуск.** Пройти preflight, тесты и ограниченный отдельно разрешённый canary именно этой конфигурации; сверить provider cost и ledger. До этого `FEATURE_APIMART_KLING_3_0_ENABLED` выключен. Отключение запрещает новые submit, но не мешает завершить старые Jobs.

## Приёмочные сценарии

- [ ] 5 с std без звука=$0.336; со звуком=$0.504.
- [ ] Цена audio=true отличается, snapshot не может совпадать с silent.
- [ ] Во второй стадии сумма длительностей 1–6 шотов равна общей duration.
- [ ] `FEATURE_APIMART_KLING_3_0_ENABLED=false` → модель недоступна для нового Job; ключ/остаток провайдера не раскрываются клиенту.
...
- [ ] Стадия 2: 4K и multi_shot; 1–6 shot, сумма длительностей равна duration. Зафиксировать audio в тарифном варианте каждой комбинации.

## Команды проверки при реализации

```powershell
go test ./internal/adapter/provider/apimart -run '^TestModel_kling_3_0$' -count=1
