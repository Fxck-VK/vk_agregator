# 50. Nano Banana 2 Lite — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** подключить Nano Banana 2 Lite через выбранный маршрут APIMart.
...
- [Карточка Study24](https://study24.ai/chat/banana_2lite)
- [Документация gemini-3.1-flash/generation-lite](https://docs.apimart.ai/ru/api-reference/images/gemini-3.1-flash/generation-lite)
...
| Поле | Значение |
| --- | --- |
| Название Study24 | Nano Banana 2 Lite |
| Предлагаемый public ID | `nano_banana_2_lite` |
| APIMart model ID | `gemini-3.1-flash-lite-image-ext` |
| Метод | `POST https://api.apimart.ai/v1/images/generations` |
| Флаг | `FEATURE_APIMART_NANO_BANANA_2_LITE_ENABLED` |
| Соответствие | Название модели сопоставлено с каталогом/контрактом APIMart; доступ рабочего ключа ещё не проверен |

Alias nano-banana-2-lite-ext документирован. Проверить фактический flat-тариф ключевой группы перед включением.
...
Только 1K: переданные 2K/4K/0.5K провайдер может тихо понизить. Наш backend отклоняет их. До 14 референсов; n=1 на старте; поиск, маска и fallback недоступны.
...
$0.0125 за изображение по прайсу. Описание upstream token pricing не использовать как итоговую ставку APIMart.
...
- Создать fixture `internal/adapter/provider/apimart/testdata/contracts/nano_banana_2_lite.request.json`.
- Создать `internal/adapter/provider/apimart/model_nano_banana_2_lite_test.go` с именем теста `TestModel_nano_banana_2_lite`.

## Порядок реализации

- [ ] **1. Контракт.** В B0 проверить `gemini-3.1-flash-lite-image-ext`, записать разрешённые поля/операции и очищенный fixture указанного запроса. Для спорной версии сохранить предлагаемое полное название в UI и закрыть rollout до подтверждения выбора. Не выводить статус «подключено» по одному наличию в прайсе.
- [ ] **2. Адаптер.** Реализовать serializer и capability для `gemini-3.1-flash-lite-image-ext` по указанному JSON; локально проверять лимиты. Разобрать описанную форму результата. В тестовом HTTP server проверить exact endpoint/body; неподдерживаемое значение не должно вызывать HTTP.
- [ ] **3. Каталог и стоимость.** Добавить/обновить public ID `nano_banana_2_lite`, backend readiness и `FEATURE_APIMART_NANO_BANANA_2_LITE_ENABLED` в config wiring. Создать отдельные ключи всех открываемых вариантов; определить exact rates и reserve/capture fixture. Сохранить model ID и тариф в Job snapshot.
...
- [ ] **6. Выпуск.** Пройти preflight, тесты и ограниченный отдельно разрешённый canary именно этой конфигурации; сверить provider cost и ledger. До этого `FEATURE_APIMART_NANO_BANANA_2_LITE_ENABLED` выключен. Отключение запрещает новые submit, но не мешает завершить старые Jobs.

## Приёмочные сценарии

- [ ] 2K отклоняется локально и не выдаётся пользователю как 2K.
- [ ] Передаётся точный model ID Lite; параметры поиска не уходят.
- [ ] `FEATURE_APIMART_NANO_BANANA_2_LITE_ENABLED=false` → модель недоступна для нового Job; ключ/остаток провайдера не раскрываются клиенту.
...
```powershell
go test ./internal/adapter/provider/apimart -run '^TestModel_nano_banana_2_lite$' -count=1
