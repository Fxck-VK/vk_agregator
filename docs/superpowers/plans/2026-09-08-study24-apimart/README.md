...
**Дата исследования API: 8 сентября 2026. Сверка с кодом: 8 сентября 2026, `dev-deploy`, HEAD `cb34ded57` + локальные изменения. Статус: B0 tooling и Qwen Image 3.0 реализованы локально; live-проверки и включение остаются открыты.**

**Текущий выбор пользователя — [45. Qwen Image 3.0](./45-qwen-image-3.md).** Модель добавлена к существующему image pipeline без отдельных подготовительных B0–B3. Доступны 1K/2K, одно изображение, до трёх референсов на backend; локальная цена 15 кредитов. Флаг по умолчанию выключен. Общие улучшения B1/B2 и остальные модели не входят в эту реализацию.
...
59 планов не означают 59 новых моделей: **3 маршрута реализованы через APIMart, 1 модель реализована через PoYo и имеет отдельный план возможной миграции, 55 маршрутов APIMart остаются новыми кандидатами**. В числе последних есть DeepSeek V4 Flash: семейство уже используется через DeepInfra, но отдельного публичного маршрута APIMart нет.
...
| --- | --- | --- | --- |
| [45. Qwen Image 3.0](./45-qwen-image-3.md) | `qwen_image_3` → APIMart / `qwen-image-3.0` | Отдельный флаг, readiness, 1K/2K, до 3 refs, submit/poll, точный floor, цена 15, contract/Web/worker lifecycle tests | Проверка рабочего ключа и отдельно разрешённый canary; runtime/env не менялись |
...
- [x] **R3 → Qwen Image 3.0: локальная реализация.** По выбору пользователя выполнен [план 45](./45-qwen-image-3.md) напрямую на существующей основе. R0–R2 остаются отдельными отложенными работами. Внешняя пробная генерация и включение Qwen не выполнены.
- [ ] **R4 → следующие выбранные модели.** Остальные новые изображения; затем T для текста, V для видео, C для Motion Control, A для Suno. Специальные протоколы и расширения входов — по потребности конкретной модели.
...
| 43 | [Grok Image](./43-grok-image-1-5.md) | `grok-imagine-1.5-ext` | Условное | $0.015 за изображение EXT. grok-imagine-image ($0.016) — другой маршрут, его тариф не смешивать. |
| 44 | [Grok Image 2.0](./44-grok-image-2-0.md) | `grok-imagine-2.0-ext` | Каталог; конфликт цены | Конфликт: прайс $0.015/изображение, документация $0.08/изображение. Рабочий тариф не назначать до сверки группы/модели. |
...
- **Grok Image 2.0 EXT:** публичная таблица показывает $0.015/изображение, страница API — $0.08. До выяснения группы/канала тариф не утверждён. Этот конфликт не скрыт выбором меньшего числа. [План](./44-grok-image-2-0.md).
...
| 2 | Остальные выбранные новые изображения; Grok 2 только после устранения конфликта | Отдельные лимиты и тарифы; 41 — лишь при отдельном выборе миграции с PoYo |
...
**Текущий статус:** 3 APIMart-маршрута реализованы локально, 1 возможная миграция с PoYo и 55 новых кандидатов. Для остальных 12 позиций сохранены причины отсутствия подтверждённого API. Детали и ограничения завершённой локальной реализации Qwen — в плане 45.
