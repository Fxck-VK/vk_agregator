# 61. Kling 3.0 Turbo — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** подключить Kling 3.0 Turbo через выбранный маршрут APIMart.
...
- [Карточка Study24](https://study24.ai/chat/kling_3_turbo)
- [Документация kling-3.0-turbo/generation](https://docs.apimart.ai/ru/api-reference/videos/kling-3.0-turbo/generation)
...
| Поле | Значение |
| --- | --- |
| Название Study24 | Kling 3.0 Turbo |
| Предлагаемый public ID | `kling_3_0_turbo` |
| APIMart model ID | `kling-3.0-turbo` |
| Метод | `POST https://api.apimart.ai/v1/videos/generations` |
| Флаг | `FEATURE_APIMART_KLING_3_0_TURBO_ENABLED` |
| Соответствие | Название модели сопоставлено с каталогом/контрактом APIMart; доступ рабочего ключа ещё не проверен |

Мультишот здесь задаётся форматированным промптом; не использовать JSON multi_shot от другой модели.
...
3–15 с; 720p/1080p; текст или первый кадр. Старт 5/10/15 с. Поля mode из kling-v3 не переносить в этот контракт.
...
- Создать fixture `internal/adapter/provider/apimart/testdata/contracts/kling_3_0_turbo.request.json`.
- Создать `internal/adapter/provider/apimart/model_kling_3_0_turbo_test.go` с именем теста `TestModel_kling_3_0_turbo`.

## Порядок реализации

- [ ] **1. Контракт.** В B0 проверить `kling-3.0-turbo`, записать разрешённые поля/операции и очищенный fixture указанного запроса. Для спорной версии сохранить предлагаемое полное название в UI и закрыть rollout до подтверждения выбора. Не выводить статус «подключено» по одному наличию в прайсе.
- [ ] **2. Адаптер.** Реализовать serializer и capability для `kling-3.0-turbo` по указанному JSON; локально проверять лимиты. Разобрать описанную форму результата. В тестовом HTTP server проверить exact endpoint/body; неподдерживаемое значение не должно вызывать HTTP.
- [ ] **3. Каталог и стоимость.** Добавить/обновить public ID `kling_3_0_turbo`, backend readiness и `FEATURE_APIMART_KLING_3_0_TURBO_ENABLED` в config wiring. Создать отдельные ключи всех открываемых вариантов; определить exact rates и reserve/capture fixture. Сохранить model ID и тариф в Job snapshot.
...
- [ ] **6. Выпуск.** Пройти preflight, тесты и ограниченный отдельно разрешённый canary именно этой конфигурации; сверить provider cost и ledger. До этого `FEATURE_APIMART_KLING_3_0_TURBO_ENABLED` выключен. Отключение запрещает новые submit, но не мешает завершить старые Jobs.

## Приёмочные сценарии

- [ ] 5 с 720p=$0.572; 5 с 1080p=$0.716.
- [ ] 4K и 16 с отклоняются.
- [ ] `FEATURE_APIMART_KLING_3_0_TURBO_ENABLED=false` → модель недоступна для нового Job; ключ/остаток провайдера не раскрываются клиенту.
...
- [ ] Стадия 2: мультишот через проверенный формат промпта Turbo. Не переносить JSON параметр multi_shot из kling-v3.

## Команды проверки при реализации

```powershell
go test ./internal/adapter/provider/apimart -run '^TestModel_kling_3_0_turbo$' -count=1
