...
- [x] `RUNBOOK.md` reflects current migrations, DLQ/retry budget, migration checksums and real adapter modes.
...
- [x] YooKassa implementation chapter 2: active product catalog seed, `GET /miniapp/payment-products`, Mini App Settings top-up intent creation + redirect to `confirmation_url`, and VK Bot top-up package/contact/payment-link flow over the same `paymentservice`. Provider return/redirect remains navigation-only; credits are granted only by webhook/reconciliation ledger top-up.
- [x] YooKassa implementation chapter 3: dashboard webhook contract documented for `payment.succeeded`, `payment.canceled` and `refund.succeeded`; `cmd/provider-webhook` inboxes events into `payment_events`, returns 200 quickly, verifies payment state via provider `GetPayment` asynchronously, and posts ledger top-ups only through tx-aware `GrantWith` for paid/captured successes. Cancel/refund webhook behavior is covered by tests.
- [x] YooKassa implementation chapter 4: reconciliation test coverage now covers missed success webhooks, repeated reconciliation, late success webhooks after reconciliation, provider-side cancellation without top-up and user-closed payment pages that remain `waiting_for_user` without credit grants.
- [x] YooKassa implementation chapter 5: manual full refund MVP is protected by `POST /billing/payment-intents/{id}/refund`, requires `ADMIN_TOKEN` + `X-Idempotency-Key`, is idempotent, operates only on `succeeded` intents, refuses spent top-up credits by checking current balance plus post-top-up negative/pending ledger movements, and keeps automatic/partial refund policy blocked until lot/FIFO attribution exists.
- [x] YooKassa implementation chapter 6: 54-FZ receipt fiscal data is snapshotted onto `payment_intents` (`receipt_description`, `vat_code`, `payment_subject`, `payment_mode`) through migration `000011_payment_intent_receipt_snapshot`; payment creation retries and manual refunds use that immutable intent snapshot, while YooKassa receipt payloads still require email or phone plus item amount, VAT, subject and mode.
- [x] YooKassa implementation chapter 7: production `cmd/provider-webhook` enforces HTTPS (direct TLS or trusted reverse-proxy HTTPS headers), YooKassa Basic Auth stays isolated inside `internal/adapter/payment/yookassa`, raw provider/auth payloads are not logged, webhook security/processing/refund rollback metrics are exposed, and runbook alert/idempotency/rollback checks are documented.
- [x] YooKassa/Product Catalog Admin chapter 5: protected operator `/billing/payment-products*` routes list/create/get/update/disable catalog rows, validate positive RUB packages and safe 54-FZ receipt fields, use constant-time `X-Admin-Token` comparison on operator surfaces, return safe DTOs without provider payloads, bump `price_version` for future snapshots on price/credits/fiscal changes, and leave existing `payment_intents` immutable.
...
- [x] Mini App Figma Create tab alignment: single-page Create surface (banner, selectable `Nano Banana Pro` / `Kling` cards, reference dropzone, inline prompt composer with estimate price + send, quick-prompt chips); PR-10 Status/Result/History preserved; non-chat create errors surfaced in UI.
- [x] Mini App Figma Profile tab alignment: banner-only hero on Settings; request history collapsed by default behind expandable panel with type filters.
- [x] Mini App Create artifact preview auth: `artifactMediaUrl` / `useArtifactMediaUrl` for `<img>`/`<video>`; `preloadArtifactBlobUrl` before result screen; F5 `launchParams` bridge timeout + bootstrap loading fix.
- [x] Mini App / VK bot price sync: `PRICES=text_generate=1,image_generate=0,video_generate=50` in env templates; UI `formatCredits(0)` → «Бесплатно».
...
Полный historical аудит безопасности/архитектуры архивирован в `docs/archive/**` (read-only ревью от
...
- [x] Mini App payment history UI wiring: Settings/Profile renders authenticated `/miniapp/payments` history with safe status/amount/credits/date fields, active `waiting_for_user` continuation links, explicit create-new-payment UX and no redirect-side balance grant.
- [~] Mini App/VK bot top-up backend flow: payment domain/migration/config foundation, tx-aware `billingservice.GrantWith`, provider port, mock/YooKassa adapters, payment service, product catalog seed/admin, 54-FZ receipt snapshots, Mini App/VK Bot purchase intent UX, authenticated payment-intent/history endpoints, Mini App payment-history UI, async YooKassa webhook-to-ledger top-up processing, stale-intent reconciliation and conservative manual full-refund MVP exist; 2026-06-10 local YooKassa smoke passed successful checkout via reconciliation, webhook replay dedup, idempotent manual refund and safe Mini App history, but still needs public HTTPS YooKassa webhook delivery, explicit provider `canceled` smoke, and partial/automatic refund lot/FIFO policy.
