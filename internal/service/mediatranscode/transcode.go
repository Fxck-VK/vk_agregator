...
func readBoundedFile(path string, maxBytes int64) ([]byte, error) {
	...
	f, err := os.Open(path) // #nosec G304 -- path is generated by os.CreateTemp inside this transcode flow.
	if err != nil {
...
func (execRunner) Run(ctx context.Context, path string, args []string, stdin []byte) error {
	cmd := exec.CommandContext(ctx, path, args...) // #nosec G204 -- operator-configured ffmpeg path; fixed args; no shell; input is stdin bytes.
