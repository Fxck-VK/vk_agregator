...
func (h *Handler) Routes() http.Handler {
	...
	mux.HandleFunc("POST /web/v1/speech-jobs/prepare", h.requireUnsafePrincipal(h.prepareSpeechJob))
	mux.HandleFunc("POST /web/v1/speech-jobs/{jobID}/activate", h.requireUnsafePrincipal(h.activateSpeechJob))
	mux.HandleFunc("GET /web/v1/speech-jobs/{jobID}", h.requirePrincipal(h.getSpeechJob))
	mux.HandleFunc("GET /web/v1/speech-jobs/{jobID}/result", h.requirePrincipal(h.getSpeechJobResult))
	mux.HandleFunc("GET /web/v1/speech-artifacts/{artifactID}", h.requirePrincipal(h.getSpeechArtifact))
...
func (h *Handler) listTextModels(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Cache-Control", "no-store")
	writeJSON(w, http.StatusOK, map[string]any{"items": h.availableTextModels()})
}
