...
func TestCreateWebConversationMessageRequestsTitleOnlyWhileConversationIsAutoPending(t *testing.T) {
	for _, testCase := range []struct {
		name      string
		origin    domain.ConversationTitleOrigin
		title     string
		wantTitle bool
	}{
		{name: "auto pending", origin: domain.ConversationTitleOriginAutoPending, wantTitle: true},
		{name: "auto fallback", origin: domain.ConversationTitleOriginAutoFallback, title: "Prompt fallback", wantTitle: false},
		{name: "auto generated", origin: domain.ConversationTitleOriginAutoGenerated, title: "Generated title", wantTitle: false},
		{name: "manual", origin: domain.ConversationTitleOriginManual, title: "Manual title", wantTitle: false},
	} {
		t.Run(testCase.name, func(t *testing.T) {
			...
			if got := jobs.inputs[0].ConversationTitleRequested; got != testCase.wantTitle {
				t.Fatalf("ConversationTitleRequested = %t, want %t", got, testCase.wantTitle)
			}
		})
	}
}
