...
func (h *Handler) process(ctx context.Context, cb callback, rawBody []byte, eventID, idemKey string, fromID, peerID int64, text, payload, ref string, attachments []vkAttachment, controlOnly bool) error {
	...
	if parsed.CreatesJob() {
		...
		job, err := h.deps.Orchestrator.CreateJob(ctx, joborchestrator.CreateJobInput{
			...
			AccountID:           accountID,
			Source:              "vk_bot",
			ChannelContext:      &domain.ChannelContext{Channel: domain.ChannelVKBot, RecipientRef: strconv.FormatInt(peerID, 10)},
			ResultMode:          domain.ResultModeExternalPush,
			DeliveryTarget:      &domain.DeliveryTarget{Channel: domain.ChannelVKBot, RecipientRef: strconv.FormatInt(peerID, 10)},
			VKPeerID:            peerID,
			CommandID:           cmd.ID,
			Operation:           parsed.Operation,
