...
func (r *ConversationRepo) RenameActiveConversationForAccount(_ context.Context, accountID, conversationID uuid.UUID, source domain.ConversationSource, title string) (*domain.Conversation, error) {
	...
	c.Title = title
	c.TitleOrigin = domain.ConversationTitleOriginManual
...
func (r *ConversationRepo) SetConversationFallbackTitleIfPending(_ context.Context, conversationID uuid.UUID, title string) (bool, error) {
	title = strings.TrimSpace(title)
	if title == "" {
		return false, nil
	}
	...
	c.Title = title
	c.TitleOrigin = domain.ConversationTitleOriginAutoFallback
	c.UpdatedAt = time.Now()
	r.byID[conversationID] = c
	return true, nil
}

func (r *ConversationRepo) SetGeneratedTitleForActiveWebConversation(_ context.Context, accountID, conversationID uuid.UUID, title string) (bool, error) {
	title = strings.TrimSpace(title)
	...
	defer r.mu.Unlock()
	c, ok := r.byID[conversationID]
	if !ok || c.AccountID != accountID || c.Source != domain.ConversationSourceWeb || c.Status != domain.ConversationActive {
		return false, nil
	}
	if c.TitleOrigin != domain.ConversationTitleOriginAutoPending && c.TitleOrigin != domain.ConversationTitleOriginAutoFallback {
		return false, nil
	}
	c.Title = title
	c.TitleOrigin = domain.ConversationTitleOriginAutoGenerated
	c.UpdatedAt = time.Now()
	r.byID[conversationID] = c
	return true, nil
}

func (r *ConversationRepo) GetUserMessageByJobID(_ context.Context, jobID uuid.UUID) (*domain.ConversationMessage, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	id, ok := r.messageByRole[messageRoleKey(jobID, domain.ConversationRoleUser)]
	if !ok {
		return nil, domain.ErrNotFound
	}
	message := r.messagesByID[id]
	return &message, nil
}

func (r *ConversationRepo) GetFirstUserMessage(_ context.Context, conversationID uuid.UUID) (*domain.ConversationMessage, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	var first *domain.ConversationMessage
	for _, id := range r.byConversation[conversationID] {
		message := r.messagesByID[id]
		if message.Role != domain.ConversationRoleUser || (first != nil && message.Seq >= first.Seq) {
			continue
		}
		copy := message
		first = &copy
