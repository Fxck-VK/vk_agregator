...
func duePreparedWebImage(job domain.Job, accountID *uuid.UUID, now time.Time) bool {
	return (accountID == nil || job.AccountID == *accountID) &&
		...
		job.Source == "web" &&
		((job.OperationType == domain.OperationImageGenerate && job.Modality == domain.ModalityImage) || ((job.OperationType == domain.OperationAudioMusic || job.OperationType == domain.OperationAudioTTS) && job.Modality == domain.ModalityAudio) || (job.OperationType == domain.OperationAudioSTT && job.Modality == domain.ModalityText)) &&
